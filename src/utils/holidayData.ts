import { Holiday } from '../types';

export const holidays2026: Holiday[] = [
  // 1월
  { date: '2026-01-01', name: '신정', type: 'public' },
  { date: '2026-01-24', name: '설날 연휴', type: 'public' },
  { date: '2026-01-25', name: '설날', type: 'public' },
  { date: '2026-01-26', name: '설날 연휴', type: 'public' },
  { date: '2026-01-27', name: '대체공휴일(설날)', type: 'public' }, // 일요일 대체

  // 3월
  { date: '2026-03-01', name: '삼일절', type: 'public' },
  { date: '2026-03-02', name: '대체공휴일(삼일절)', type: 'public' }, // 일요일 대체

  // 5월
  { date: '2026-05-05', name: '어린이날', type: 'public' },
  { date: '2026-05-24', name: '부처님오신날', type: 'public' },
  { date: '2026-05-25', name: '대체공휴일(부처님오신날)', type: 'public' }, // 일요일 대체

  // 6월
  { date: '2026-06-06', name: '현충일', type: 'public' },

  // 8월
  { date: '2026-08-15', name: '광복절', type: 'public' },

  // 9월 추석
  { date: '2026-09-26', name: '추석 연휴', type: 'public' },
  { date: '2026-09-27', name: '추석', type: 'public' },
  { date: '2026-09-28', name: '추석 연휴', type: 'public' },
  { date: '2026-09-29', name: '대체공휴일(추석)', type: 'public' }, // 일요일 대체

  // 10월
  { date: '2026-10-03', name: '개천절', type: 'public' },
  { date: '2026-10-09', name: '한글날', type: 'public' },

  // 12월
  { date: '2026-12-25', name: '크리스마스', type: 'public' },
];
